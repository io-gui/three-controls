...
export class PointerEvents {
	constructor(domElement, params = {}) {
		...
		function _onContextmenu(event) {
			...
			scope.dispatchEvent("contextmenu", { });
		}
		...
		function _onMouseDown(event) {
			...
			if (!dragging) {
				...
				scope.dispatchEvent("pointerdown", makePointerEvent("pointerdown", scope.pointers));
			}
		}
		function _onMouseMove(event) {
			event.preventDefault();
			scope.pointers.update(event, "pointermove");
			scope.dispatchEvent("pointermove", makePointerEvent("pointermove", scope.pointers));
		}
		function _onMouseHover(event) {
			...
			scope.dispatchEvent("pointerhover", makePointerEvent("pointerhover", scope.pointers));
		}
		function _onMouseUp(event) {
			...
			if (event.buttons === 0) {
				...
				scope.dispatchEvent("pointerup", makePointerEvent("pointerup", scope.pointers));
			}
		}
	...
	dispatchEvent(type, event) {
		...
		if (this._listeners[type] !== undefined) {
			...
			var array = this._listeners[type].slice(0);
