...
import { ControlsMixin, CONTROL_CHANGE_EVENT, CONTROL_START_EVENT, CONTROL_END_EVENT } from './Controls';
...
class TransformControls extends ControlsMixin( TransformHelper ) {
	...
	onTrackedPointerHover( pointer ) {
		...
		if ( ! this.object || this.dragging === true )
			return;

		const pickers = this.children.filter( ( child ) => {
	...
	onTrackedPointerMove( pointer ) {
		...
		if ( ! object || axis === '' || this.dragging === false || pointer.button !== 0 )
			return;
