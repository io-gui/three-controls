import { BoxBufferGeometry, BufferGeometry, Color, CylinderBufferGeometry, DoubleSide, Euler, Float32BufferAttribute, Line, LineBasicMaterial, Matrix4, Mesh, MeshBasicMaterial, Object3D, OctahedronBufferGeometry, PlaneBufferGeometry, Quaternion, SphereBufferGeometry, TorusBufferGeometry, Vector3, OrthographicCamera } from "../../../three";
import { ControlsMixin, CHANGE_EVENT, START_EVENT, END_EVENT } from "./Controls.js";
...
class TransformControls extends ControlsMixin( Object3D ) {
	...
	onTrackedPointerMove( pointer ) {
		...
		if ( object === undefined || axis === '' || this.dragging === false || pointer.button !== 0 )
			return;
		const planeIntersect = getFirstIntersection( pointer.intersectObjects( [ this._plane ] ), true );
		if ( ! planeIntersect )
			return;
...
class TransformControlsPlane extends Mesh {
	...
	constructor() {
		...
		super( new PlaneBufferGeometry( 100000, 100000, 2, 2 ), new MeshBasicMaterial( { visible: false, wireframe: true, side: DoubleSide, transparent: true, opacity: 0.1, toneMapped: false } ) );
